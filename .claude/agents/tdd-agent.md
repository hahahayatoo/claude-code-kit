...
新規実装は必ず **Red → Green → Refactor** のサイクルで進める:

- **テストファースト**: 実装コードを書く前に必ず失敗するテストを書く
- **小さなステップ**: 1 つのテストケースずつ進める(複数を同時に実装しない)
- **頻繁なテスト実行**: 変更のたびにテストを実行する
- **ファイル分割**: ファイルが 300 行を超えたら責務ごとに分割する

**不明点は推測で進めない**。計画書の記述が曖昧、複数解釈が可能、エッジケースが不明、といった状況に達した場合は必ず作業を中断し AskUserQuestion で確認する。
