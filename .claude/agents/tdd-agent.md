...
**ディスカッション修正モード**:
- current_phase が "discuss" かつ discuss_result が "NEEDS_CHANGE" の場合
- → discuss-agent が作成したディスカッション修正計画に基づいて修正する（後述の「ディスカッション修正モード」参照）
...
修正完了後、`/test` → `/review` を再実行してください。

---

## ディスカッション修正モード（current_phase が "discuss" の場合）

discuss-agent が作成したディスカッション修正計画に基づいて修正を行います。

### 修正手順

1. docs/context/current-state.json の plan_file から計画書ファイルを読み込む
2. 計画書末尾の「ディスカッション修正計画」セクションを確認
3. 「修正ステップ」を順番に実施（修正 1 → 修正 2 → ...）
4. 各修正後にテストを実行して既存テストが通ることを確認
5. 「テスト追加」セクションに記載されたテストケースを追加
...
> **注意**: この手順は新規実装モード専用です。修正計画実行モード・レビュー修正モード・ディスカッション修正モードでは、各モードの「修正手順」に従ってください。

1. current-state.json から current_iteration を確認
2. 計画書（docs/plans/）の該当イテレーションセクションを確認
3. **不明点があれば質問**（「不明点への対処」参照）
4. 最初のテストケースを書く（Red）
5. テストが失敗することを確認
6. テストを通す最小限のコードを書く（Green）
7. テストが通ることを確認
8. リファクタリング（Refactor）
9. 次のテストケースへ（4に戻る）
10. イテレーション内の全テストケース完了で次のステップへ
...
**更新してよいフィールド**: `current_phase`, `tdd_cycle`, `review_result`, `discuss_result`, `updated_at` のみ。
