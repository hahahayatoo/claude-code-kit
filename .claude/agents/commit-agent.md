...
以下のいずれかに該当する場合、**コミットを即座に中止する**(コードを含む変更の場合):

- `last_test_result` が `"pass"` ではない
- `review_result` が `"PASS"` ではない
- `.env` ファイルやシークレットを含むファイルが対象に含まれる

ドキュメントのみの変更(全変更が `.md` または `docs/` 配下)は、テスト/レビュー結果のチェックをスキップしてよい。ただし `.env`/シークレットチェックは常に実行する。
